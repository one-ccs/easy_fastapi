"""SQLModel 运行时扩展（provide db_session_factory/model_introspector/user_model/role_model）。

从通用 database 段读取连接配置（ORM 无关），自行转换为 SQLModel 连接 URL。
...
from .introspector import SQLModelModelIntrospector
...
class SQLModelExtension:
    ...
    def init_app(self, app: FastAPI, config, ctx: ExtensionContext) -> None:
        if not self._models:
            raise ValueError("SQLModelExtension 需要传入 models=[User, Role, ...]（项目实体类）")
        loader = ctx.get_loader()
        if loader:
            db_url = build_db_url("sqlmodel", loader)
        else:
            db_url = build_uri("sqlmodel", DatabaseConfig())
        ...
        _factory = sessionmaker(engine, class_=_SQLModelAsyncSession, expire_on_commit=False)
        ...
        ctx.provide("model_introspector", SQLModelModelIntrospector())
