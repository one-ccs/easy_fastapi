...
# ---- Minimal fakes satisfying UserModelProtocol / RoleModelProtocol ----


class FakeUser:
    """满足 UserModelProtocol 的最小假类。"""
...
def test_extension_name():
    assert SQLAlchemyExtension(models=[FakeUser, FakeRole]).name == "orm.sqlalchemy"


def test_extension_config_model():
    assert SQLAlchemyExtension(models=[FakeUser, FakeRole]).config_model() is None
...
def test_extension_requires_empty():
    assert SQLAlchemyExtension(models=[FakeUser, FakeRole]).requires == []
...
def test_init_app_uses_build_db_url_when_loader_present(tmp_path):
    ...
    # build_db_url("sqlalchemy", loader) → postgresql+asyncpg://alice:secret@db.local:5432/mydb
...
def test_init_app_calls_build_db_url_with_correct_orm_name(tmp_path):
    """验证调用 build_db_url 时传入的 orm 参数为 'sqlalchemy'。"""
    with patch(
        "easy_fastapi.ext.orm.sqlalchemy.extension.build_db_url",
...
def test_build_db_url_private_function_removed():
    ...
    import easy_fastapi.ext.orm.sqlalchemy.extension as ext_mod
